-- Reassign one product on an order to inventory that actually remains.
-- Scoped to (order, package) so multi-event baskets are safe; do not use
-- admin_set_order_cost_allocations here (that RPC is whole-order guests).
...
begin
  ...
  if exists (
    ...
    where l.id is null
      or (l.package_id is distinct from p_package_id and l.package_id is distinct from v_ledger)
  ) then
    raise exception 'invalid_cost_layer_for_package';
  end if;

  -- Return this product's current take, including layers on the linked 3-day ledger.
  ...
  from (
    ...
    where occ.order_id = p_order_id
      ...
      and (
        occ.package_id = p_package_id
        or exists (
          select 1
          from public.package_cost_layers layer
          where layer.id = occ.cost_layer_id
            and layer.package_id in (p_package_id, v_ledger)
        )
      )
  ...
  delete from public.order_cost_consumptions occ
  where occ.order_id = p_order_id
    and (
      occ.package_id = p_package_id
      or (
        occ.cost_layer_id is not null
        and exists (
          select 1
          from public.package_cost_layers layer
          where layer.id = occ.cost_layer_id
