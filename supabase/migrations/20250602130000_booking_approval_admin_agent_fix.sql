-- Allow admin-on-behalf orders when approving requests (incl. admin test accounts).
-- Portal requests may only be submitted by approved agents or admins.
...
begin
  ...
  select (p.approval_status = 'approved'), p.role, coalesce(nullif(btrim(p.company_name), ''), '')
  into v_approved, v_role, v_company
...
-- place_order: when admin books on behalf (approve / place order), allow approved agent OR admin profile
