-- Allow admin order supplier reallocation against the linked 3-day cost ledger
-- (day packages share the parent purchase layers).
...
begin
  ...
  v_ledger := public.resolve_cost_ledger_package_id(v_order.package_id);
  ...
  if exists (
    ...
    where l.id is null
      or (l.package_id is distinct from v_order.package_id and l.package_id is distinct from v_ledger)
  ) then
...
grant execute on function public.admin_set_order_cost_allocations(uuid, jsonb) to service_role;
