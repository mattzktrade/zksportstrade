-- Reverse a mistaken finance "mark paid" without touching later fulfilment stages.

create or replace function public.admin_mark_finance_unpaid(
...
begin
  ...
  if v_deal_id is not null then
    ...
    where id = v_deal_id
      and stage = 'paid_confirmed';
  end if;

  -- Invoice→ops trigger never regresses confirmed fulfilment; undo that locally.
  if v_order_id is not null then
    ...
    set
      ...
      updated_at = timezone('utc', now())
    where order_id = v_order_id
      and fulfilment_status = 'confirmed';
  end if;
end;
$$;

revoke all on function public.admin_mark_finance_unpaid(uuid, uuid) from public;
grant execute on function public.admin_mark_finance_unpaid(uuid, uuid) to authenticated;
