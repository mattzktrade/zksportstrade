-- Multi-event deal baskets with line-level owned/brokered sourcing.

alter table public.deal_line_items
  add column if not exists sourcing_mode text not null default 'owned',
  add column if not exists supplier_quote_at timestamptz,
  add column if not exists sourcing_shortage_id uuid;

alter table public.deal_line_items
  drop constraint if exists deal_line_items_sourcing_mode_check;
alter table public.deal_line_items
  add constraint deal_line_items_sourcing_mode_check
  check (sourcing_mode in ('owned', 'brokered'));

alter table public.sourcing_shortages
  add column if not exists deal_line_item_id uuid
    references public.deal_line_items(id) on delete cascade;
create unique index if not exists sourcing_shortages_deal_line_unique_idx
  on public.sourcing_shortages(deal_line_item_id)
  where deal_line_item_id is not null and status <> 'cancelled';

alter table public.deal_line_items
  drop constraint if exists deal_line_items_sourcing_shortage_fkey;
alter table public.deal_line_items
  add constraint deal_line_items_sourcing_shortage_fkey
  foreign key (sourcing_shortage_id) references public.sourcing_shortages(id) on delete set null;

alter table public.order_line_items
  add column if not exists sourcing_mode text not null default 'owned',
  add column if not exists supplier_id uuid references public.suppliers(id) on delete set null,
  add column if not exists expected_unit_cost numeric;

alter table public.order_line_items
  drop constraint if exists order_line_items_sourcing_mode_check;
alter table public.order_line_items
  add constraint order_line_items_sourcing_mode_check
  check (sourcing_mode in ('owned', 'brokered'));
...
create or replace function public.adjust_linked_inventory_held(
  p_package_id text,
  p_delta int
)
returns void
...
begin
  ...
  set qty_held = greatest(0, coalesce(qty_held, 0) + p_delta)
  where package_id = p_package_id;
  if v_group is null then return; end if;

  if v_duration = '3_day' then
    update public.package_inventory inventory
    set qty_held = greatest(0, coalesce(inventory.qty_held, 0) + p_delta)
    from public.packages package
    where inventory.package_id = package.id
      and package.inventory_group_id = v_group
      and package.id <> p_package_id
      and package.duration in ('3_day', 'friday_only', 'saturday_only', 'sunday_only');
    ...
    where inventory.package_id = package.id
      and package.inventory_group_id = v_group
      and package.id <> p_package_id
      and package.duration in ('2_day', 'saturday_only', 'sunday_only');
  else
    update public.package_inventory inventory
    set qty_held = greatest(0, coalesce(inventory.qty_held, 0) + p_delta)
    from public.packages package
    where inventory.package_id = package.id
      and package.inventory_group_id = v_group
      and package.id <> p_package_id
      and package.duration = v_duration;
  end if;
end;
$$;
...
begin
  ...
  if p_release_status not in ('released', 'expired', 'cancelled') then raise exception 'invalid_release_status'; end if;
  if nullif(btrim(p_reason), '') is null then raise exception 'reason_required'; end if;
  if not exists (select 1 from public.deals where id = p_deal_id) then raise exception 'deal_not_found'; end if;

  for v_res in
    select id, package_id, pool_id, quantity, kind
    from public.inventory_reservations
    where deal_id = p_deal_id and status = 'active'
    order by created_at, id
    for update
  loop
    if v_res.kind = 'deal_reservation' then
      perform public.lock_package_inventory(v_res.package_id);
      perform public.adjust_linked_inventory_held(v_res.package_id, -v_res.quantity);
      insert into public.inventory_ledger_entries (
        package_id, pool_id, entry_type, quantity_delta, reason, actor_profile_id,
        source_table, source_id, reservation_id, deal_id, metadata
      ) values (
        v_res.package_id, v_res.pool_id, 'reservation_release', v_res.quantity,
        btrim(p_reason), auth.uid(), 'inventory_reservations', v_res.id::text,
        v_res.id, p_deal_id, jsonb_build_object('release_status', p_release_status)
      )
      on conflict (source_table, source_id, entry_type)
        where source_table is not null and source_id is not null do nothing;
    else
      update public.sourcing_shortages shortage
      set status = case when shortage.status = 'purchased' then shortage.status else 'cancelled' end,
          cleared_at = case when shortage.status = 'purchased' then shortage.cleared_at else timezone('utc', now()) end,
          updated_at = timezone('utc', now())
      where shortage.deal_id = p_deal_id
        and shortage.package_id = v_res.package_id;
    end if;

    update public.inventory_reservations
    set status = p_release_status, released_at = timezone('utc', now()),
        updated_at = timezone('utc', now()), note = concat_ws(E'\n', note, btrim(p_reason))
    where id = v_res.id;
    update public.deal_line_items
    set reservation_status = p_release_status, updated_at = timezone('utc', now())
    where reservation_id = v_res.id and reservation_status = 'active';
    v_released := v_released + 1;
  end loop;

  if v_released > 0 then
    update public.deals
    set hold_expires_at = null, do_not_expire = false, updated_at = timezone('utc', now())
    where id = p_deal_id;
    insert into public.deal_activities (deal_id, actor_profile_id, action, summary, metadata)
    values (
      p_deal_id, auth.uid(), 'reservation_' || p_release_status, btrim(p_reason),
...
create or replace function public.validate_brokered_lines_before_booking_form()
returns trigger
language plpgsql
security definer
set search_path = public
...
begin
  if exists (
    select 1
    from public.deal_line_items line
    where line.deal_id = new.deal_id
      and line.sourcing_mode = 'brokered'
      and (
        line.supplier_id is null
        or line.expected_unit_cost is null
        or line.supplier_quote_at is null
        or line.supplier_quote_at < timezone('utc', now()) - interval '24 hours'
        or line.supplier_quote_at > timezone('utc', now()) + interval '5 minutes'
      )
  ) then
    raise exception 'brokered_line_requires_fresh_supplier_quote';
  end if;
  return new;
end;
$$;

drop trigger if exists booking_forms_brokered_quote_guard_trg on public.booking_forms;
create trigger booking_forms_brokered_quote_guard_trg
before insert on public.booking_forms
for each row execute function public.validate_brokered_lines_before_booking_form();
...
begin
  ...
  loop
    ...
    if exists (
      select 1 from jsonb_array_elements(v_snapshot->'lines') duplicate
      where duplicate->>'packageId' = v_package_id
      group by duplicate->>'packageId' having count(*) > 1
    ) then raise exception 'duplicate_package_lines_not_supported'; end if;
  ...
  loop
    ...
    if not found then raise exception 'deal_line_missing:%', v_package_id; end if;

    select * into v_res
    from public.inventory_reservations
    where deal_id = v_deal.id and package_id = v_package_id and status = 'active'
...
declare
  ...
  v_cons record;
begin
  if not public.has_cms_permission('finance.manage') and not public.is_admin() then raise exception 'forbidden'; end if;
  if nullif(btrim(p_reason), '') is null then raise exception 'reason_required'; end if;
  ...
  if v_invoice.status in ('paid', 'delivered') then raise exception 'paid_or_delivered_order_cannot_be_cancelled'; end if;
  ...
  for v_cons in
    select cost_layer_id, quantity from public.order_cost_consumptions where order_id = v_order.id
  loop
  ...
  where order_id = v_order.id and status <> 'tickets_received';
  update public.orders set status = 'cancelled' where id = v_order.id;
  update public.invoices
  set status = 'cancelled', cancelled_at = timezone('utc', now()), reconciliation_note = btrim(p_reason)
  where order_id = v_order.id;
  update public.deals
  set stage = 'cancelled', next_action = null, next_action_due_at = null,
      closed_at = timezone('utc', now()), updated_at = timezone('utc', now())
  ...
  return jsonb_build_object(
    'order_id', v_order.id, 'deal_id', v_order.deal_id, 'already_cancelled', false
  );
end;
$$;
