-- Issue date is recorded when admin moves invoice to awaiting_payment, not at checkout.
...
comment on column public.invoices.issued_at is
  'Set when status becomes awaiting_payment (invoice issued to agent). Null while awaiting_invoice.';

-- place_order: new invoices have no issue date until finance issues them
