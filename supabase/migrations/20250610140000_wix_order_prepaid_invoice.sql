-- Wix checkout is card-paid at source: portal invoice is paid immediately (no Xero ACCREC / email).
...
comment on function public.place_wix_order is
  'Service-role only: record a prepaid Wix order at retail price; portal invoice is paid (no Xero invoice email).';
