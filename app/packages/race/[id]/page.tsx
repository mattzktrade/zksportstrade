...
export default function RacePackagesPage({ params }: { params: Promise<{ id: string }> }) {
  const { id } = use(params)
  const race = races2026.find((r) => r.id === id)
...
function PackageRow({
  ...
  const availabilityPercent = isAvailabilityString ? 0 : (pkg.availability / pkg.totalCapacity) * 100
  const totalPrice = pkg.price ? pkg.price * guestCount : 0
  const maxGuests = isAvailabilityString ? 1 : Math.min(pkg.availability, pkg.totalCapacity)
  const canBook = !isAvailabilityString && pkg.price !== null && pkg.availability > 0
