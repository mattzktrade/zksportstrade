      ...
      <div className="p-3 sm:p-4 space-y-2 sm:space-y-3">
        ...
        <div className="flex items-center justify-between pt-2 border-t border-border">
          <div>
            ...
            <p className="text-xs sm:text-sm font-medium text-foreground">{race.packageCount} available</p>
          </div>
...
export function PackagesPageClient({ catalog }: { catalog: PortalCatalog }) {
  const [seasonYear, setSeasonYear] = useState<PortalCatalogSeasonYear>(catalog.defaultSeasonYear)
  const [search, setSearch] = useState("")

  const slice = getSeasonSlice(catalog, seasonYear) ?? catalog.seasons[0]
  const races = slice?.races ?? []
  const packages = slice?.packages ?? []
  ...
  const racesWithPackages = useMemo(() => {
    const filtered = races.filter((race) => {
      ...
      return (
        ...
        race.shortName.toLowerCase().includes(q) ||
    ...
    return filtered
      .map((race) => {
        const racePackages = packages.filter((pkg) => pkg.raceId === race.id)
      ...
      <div className="flex flex-col gap-4">
        <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
          <div className="flex flex-col gap-3 min-w-0">
            <h1 className="text-2xl sm:text-3xl font-bold text-foreground tracking-tight">Packages</h1>
            <CatalogSeasonTabs
              seasons={seasonTabs}
      ...
      {racesWithPackages.length === 0 ? (
        ...
        <>
              ...
              <table className="w-full">
                <thead>
                  <tr className="bg-muted/30 border-b border-border">
                    ...
                    <th className="text-right px-4 sm:px-6 py-3 sm:py-4 text-xs sm:text-sm font-medium text-muted-foreground" />
                  </tr>
                </thead>
                <tbody className="divide-y divide-border">
                  {racesWithPackages.map((race) => (
                    <RaceTableRow key={race.id} race={race} />
                  ))}
                </tbody>
              </table>
            </div>
          ...
          <div className="lg:hidden space-y-3 sm:space-y-4">
            {racesWithPackages.map((race) => (
              <RaceMobileCard key={race.id} race={race} />
            ))}
          </div>
        </>
      )}
